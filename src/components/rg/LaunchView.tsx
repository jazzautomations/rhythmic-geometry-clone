...
const MODE_PREVIEW_ORBITS = {
  ...
  "polyrhythm-study": [
    { pulseCount: 2, radius: 35, color: "#00FFAA", direction: 1 as const, offsetTurns: 0 },
    { pulseCount: 3, radius: 70, color: "#7FD7FF", direction: -1 as const, offsetTurns: 0.25 },
  ],
  "riff-cycle-study": [
    { pulseCount: 4, radius: 40, color: "#FFD166", direction: 1 as const, offsetTurns: 0 },
    { pulseCount: 7, radius: 80, color: "#FF7799", direction: -1 as const, offsetTurns: 0.3 },
    { pulseCount: 11, radius: 120, color: "#AA88FF", direction: 1 as const, offsetTurns: 0.1 },
  ],
};
  ...
  return (
    <div className="relative min-h-screen bg-[#090a10] text-white">
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute inset-0 opacity-85 rg-glow-aurora" />
        <div className="absolute inset-0 opacity-20 rg-grid-bg" />
      </div>
      ...
      <main className="px-5 pb-16 pt-10 sm:px-8 sm:pb-20 sm:pt-14">
        <div className="mx-auto max-w-7xl">
          ...
          >
            <h1 className="rg-font-serif text-5xl font-light tracking-[-0.05em] leading-[0.92] sm:text-6xl lg:text-[5.2rem]">
              Geometry Modes
            </h1>
          </motion.div>
              ...
              >
                ...
                <div className="relative mt-6 flex-1 overflow-hidden rounded-[1.9rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.01))] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_30px_90px_rgba(0,0,0,0.34)]">
                  <div className="h-full overflow-hidden rounded-[1.45rem] bg-[radial-gradient(circle_at_50%_40%,rgba(255,255,255,0.06),rgba(9,10,16,0.22)_52%,rgba(9,10,16,0.92)_100%)]">
                    <OrbitPreview
                      seed={m.id}
                      orbits={MODE_PREVIEW_ORBITS[m.id]}
                      bpm={m.id === "riff-cycle-study" ? 110 : m.id === "polyrhythm-study" ? 96 : 84}
                    />
                  </div>
                </div>
          ...
          <div className="mt-10 grid gap-4 lg:grid-cols-[1.08fr_0.92fr]">
            <div className="rounded-[1.8rem] border border-white/[0.08] bg-white/[0.03] px-6 py-6 text-sm leading-8 text-white/52 sm:px-7">
              ...
              <div className="mt-4 space-y-3">
                <p>
                  Start with <span className="text-white/78">Orbits</span> if you want the strongest first
                  impression.
                </p>
                <p>
                  Start with <span className="text-white/78">Polyrhythm Study</span> if you want to hear how
                  layered rhythms meet.
                </p>
                <p>
                  Start with <span className="text-white/78">Riff Cycle</span> if you already want to write
                  phrases against a bar frame.
                </p>
              </div>
            </div>
            <div className="rounded-[1.8rem] border border-white/[0.08] bg-[#0c0f16] px-6 py-6 text-sm leading-8 text-white/54 sm:px-7">
              ...
              <p className="mt-4">
                The choice is not permanent. The app opens on one surface first, but the system stays connected
                once you are inside — switch modes from the top nav at any time.
              </p>
            </div>
          </div>
        </div>
